...
object Redis {
  ...
  private[redis4cats] def acquireAndRelease[F[_]: Concurrent: ContextShift: Log, K, V](
    ...
    val acquire = txBlocker
      .blockOn {
        txBlocker.delay(client.underlying.connect(codec.underlying, client.uri.underlying))
      }
  ...
  private[redis4cats] def acquireAndReleaseCluster[F[_]: Concurrent: ContextShift: Log, K, V](
    ...
    val acquire = txBlocker
      .blockOn {
        txBlocker.delay(client.underlying.connect[K, V](codec.underlying))
      }
...
private[redis4cats] class BaseRedis[F[_]: Concurrent: ContextShift, K, V](
  ...
  def sync: F[RedisClusterSyncCommands[K, V]] =
    txBlocker.blockOn(if (cluster) conn.clusterSync else conn.sync.widen)

  /******************************* Keys API *************************************/
  def del(key: K*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.del(key: _*))).void
    }
  ...
  def expire(key: K, expiresIn: FiniteDuration): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.expire(key, expiresIn.toSeconds))).void
    }
  ...
  def multi: F[Unit] =
    txBlocker.blockOn {
      sync.flatMap {
        case c: RedisSyncCommands[K, V] => F.delay(c.multi())
        case _                          => conn.sync.flatMap(c => F.delay(c.multi()))
      }.void
    }
  ...
  def exec: F[TransactionResult] =
    txBlocker.blockOn {
      sync
        .flatMap {
          case c: RedisSyncCommands[K, V] => F.delay(c.exec())
          case _                          => conn.sync.flatMap(c => F.delay(c.exec()))
        }
        .flatMap {
          case res if res.wasDiscarded() => F.raiseError(TransactionDiscarded)
          case res: TransactionResult    => res.pure[F]
        }
    }
  ...
  def discard: F[Unit] =
    txBlocker.blockOn {
      sync.flatMap {
        case c: RedisSyncCommands[K, V] => F.delay(c.discard())
        case _                          => conn.sync.flatMap(c => F.delay(c.discard()))
      }.void
    }
  ...
  def watch(keys: K*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap {
        case c: RedisSyncCommands[K, V] => F.delay(c.watch(keys: _*))
        case _                          => conn.sync.flatMap(c => F.delay(c.watch(keys: _*)))
      }.void
    }
  ...
  def unwatch: F[Unit] =
    txBlocker.blockOn {
      sync.flatMap {
        case c: RedisSyncCommands[K, V] => F.delay(c.unwatch())
        case _                          => conn.sync.flatMap(c => F.delay(c.unwatch()))
      }.void
    }

  /******************************* AutoFlush API **********************************/
  // Pipelining only works with the sync API
  ...
  override def append(key: K, value: V): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.append(key, value))).void
    }
  ...
  override def set(key: K, value: V): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.set(key, value))).void
    }
  ...
  override def set(key: K, value: V, setArgs: SetArgs): F[Boolean] = {
    ...
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.set(key, value, jSetArgs))).map(_ == "OK")
    }
  }
  ...
  override def setEx(key: K, value: V, expiresIn: FiniteDuration): F[Unit] = {
    val command = expiresIn.unit match {
      case TimeUnit.MILLISECONDS =>
        sync.flatMap(c => F.delay(c.psetex(key, expiresIn.toMillis, value)))
      case _ =>
        sync.flatMap(c => F.delay(c.setex(key, expiresIn.toSeconds, value)))
    }
    txBlocker.blockOn(command.void)
  }

  override def setRange(key: K, value: V, offset: Long): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.setrange(key, offset, value))).void
    }
  ...
  override def mSet(keyValues: Map[K, V]): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.mset(keyValues.asJava))).void
    }
  ...
  override def bitOpAnd(destination: K, sources: K*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.bitopAnd(destination, sources: _*))).void
    }

  override def bitOpNot(destination: K, source: K): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.bitopNot(destination, source))).void
    }

  override def bitOpOr(destination: K, sources: K*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.bitopOr(destination, sources: _*))).void
    }

  override def bitOpXor(destination: K, sources: K*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.bitopXor(destination, sources: _*))).void
    }

  /******************************* Hashes API **********************************/
  override def hDel(key: K, fields: K*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.hdel(key, fields: _*))).void
    }
  ...
  override def hSet(key: K, field: K, value: V): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.hset(key, field, value))).void
    }
  ...
  override def hmSet(key: K, fieldValues: Map[K, V]): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.hmset(key, fieldValues.asJava))).void
    }
  ...
  override def sAdd(key: K, values: V*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.sadd(key, values: _*))).void
    }

  override def sDiffStore(destination: K, keys: K*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.sdiffstore(destination, keys: _*))).void
    }

  override def sInterStore(destination: K, keys: K*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.sinterstore(destination, keys: _*))).void
    }

  override def sMove(source: K, destination: K, value: V): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.smove(source, destination, value))).void
    }
  ...
  override def sRem(key: K, values: V*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.srem(key, values: _*))).void
    }
  ...
  override def sUnionStore(destination: K, keys: K*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.sunionstore(destination, keys: _*))).void
    }
  ...
  override def lPush(key: K, values: V*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.lpush(key, values: _*))).void
    }

  override def lPushX(key: K, values: V*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.lpushx(key, values: _*))).void
    }
  ...
  override def rPush(key: K, values: V*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.rpush(key, values: _*))).void
    }

  override def rPushX(key: K, values: V*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.rpushx(key, values: _*))).void
    }

  override def lInsertAfter(key: K, pivot: V, value: V): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.linsert(key, false, pivot, value))).void
    }

  override def lInsertBefore(key: K, pivot: V, value: V): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.linsert(key, true, pivot, value))).void
    }

  override def lRem(key: K, count: Long, value: V): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.lrem(key, count, value))).void
    }

  override def lSet(key: K, index: Long, value: V): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.lset(key, index, value))).void
    }

  override def lTrim(key: K, start: Long, stop: Long): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.ltrim(key, start, stop))).void
    }
  ...
  override def geoAdd(key: K, geoValues: GeoLocation[V]*): F[Unit] =
    txBlocker.blockOn {
      ...
      sync.flatMap(c => F.delay(c.geoadd(key, triplets: _*))).void
    }
  ...
  override def geoRadiusByMember(
    ...
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.georadiusbymember(key, value, dist.value, unit, storage.asGeoRadiusStoreArgs))).void
    }
  ...
  override def zAdd(key: K, args: Option[ZAddArgs], values: ScoreWithValue[V]*): F[Unit] =
    txBlocker.blockOn {
      args match {
        case Some(x) =>
          sync.flatMap(c => F.delay(c.zadd(key, x, values.map(s => ScoredValue.just(s.score.value, s.value)): _*)))
        case None =>
          sync.flatMap(c => F.delay(c.zadd(key, values.map(s => ScoredValue.just(s.score.value, s.value)): _*)))
      }
    }.void
  ...
  override def zAddIncr(key: K, args: Option[ZAddArgs], member: ScoreWithValue[V]): F[Unit] =
    txBlocker.blockOn {
      args match {
        case Some(x) => sync.flatMap(c => F.delay(c.zaddincr(key, x, member.score.value, member.value)))
        case None    => sync.flatMap(c => F.delay(c.zaddincr(key, member.score.value, member.value)))
      }
    }.void

  override def zIncrBy(key: K, member: V, amount: Double): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.zincrby(key, amount, member))).void
    }

  override def zInterStore(destination: K, args: Option[ZStoreArgs], keys: K*): F[Unit] =
    txBlocker.blockOn {
      args match {
        case Some(x) => sync.flatMap(c => F.delay(c.zinterstore(destination, x, keys: _*)))
        case None    => sync.flatMap(c => F.delay(c.zinterstore(destination, keys: _*)))
      }
    }.void

  override def zRem(key: K, values: V*): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.zrem(key, values: _*))).void
    }

  override def zRemRangeByLex(key: K, range: ZRange[V]): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.zremrangebylex(key, JRange.create[V](range.start, range.end)))).void
    }

  override def zRemRangeByRank(key: K, start: Long, stop: Long): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.zremrangebyrank(key, start, stop))).void
    }

  override def zRemRangeByScore(key: K, range: ZRange[V])(implicit ev: Numeric[V]): F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.zremrangebyscore(key, range.asJavaRange))).void
    }

  override def zUnionStore(destination: K, args: Option[ZStoreArgs], keys: K*): F[Unit] =
    txBlocker.blockOn {
      args match {
        case Some(x) => sync.flatMap(c => F.delay(c.zunionstore(destination, x, keys: _*)))
        case None    => sync.flatMap(c => F.delay(c.zunionstore(destination, keys: _*)))
      }
    }.void
    ...
    txBlocker
      .blockOn {
        limit match {
          ...
          case None => sync.flatMap(c => F.delay(c.zrangebylex(key, JRange.create[V](range.start, range.end))))
        }
      }
      .map(_.asScala.toList)
  ...
  override def zRangeByScore[T: Numeric](key: K, range: ZRange[T], limit: Option[RangeLimit]): F[List[V]] =
    txBlocker
      .blockOn {
        limit match {
          case Some(x) =>
            sync.flatMap(c => F.delay(c.zrangebyscore(key, range.asJavaRange, JLimit.create(x.offset, x.count))))
          case None => sync.flatMap(c => F.delay(c.zrangebyscore(key, range.asJavaRange)))
        }
      }
      .map(_.asScala.toList)
    ...
    txBlocker
      .blockOn {
        limit match {
          ...
          case None => sync.flatMap(c => F.delay(c.zrangebyscoreWithScores(key, range.asJavaRange)))
        }
      }
    ...
    txBlocker
      .blockOn {
        limit match {
          ...
          case None => sync.flatMap(c => F.delay(c.zrevrangebylex(key, JRange.create[V](range.start, range.end))))
        }
      }
      .map(_.asScala.toList)
  ...
  override def zRevRangeByScore[T: Numeric](key: K, range: ZRange[T], limit: Option[RangeLimit]): F[List[V]] =
    txBlocker
      .blockOn {
        limit match {
          case Some(x) =>
            sync.flatMap(c => F.delay(c.zrevrangebyscore(key, range.asJavaRange, JLimit.create(x.offset, x.count))))
          case None => sync.flatMap(c => F.delay(c.zrevrangebyscore(key, range.asJavaRange)))
        }
      }
      .map(_.asScala.toList)
  ...
  override val ping: F[String] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.ping()))
    }

  /******************************* Server API **********************************/
  override val flushAll: F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.flushall())).void
    }

  override val flushAllAsync: F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.flushallAsync())).void
    }
  ...
  override def evalSha(script: String, output: ScriptOutputType[V]): F[output.R] =
    txBlocker
      .blockOn {
        sync.flatMap(c => F.delay(c.evalsha[output.Underlying](script, output.outputType)))
      }
      .map(output.convert(_))
  ...
  override def scriptLoad(script: V): F[String] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.scriptLoad(script)))
    }
  ...
  override def scriptFlush: F[Unit] =
    txBlocker.blockOn {
      sync.flatMap(c => F.delay(c.scriptFlush())).void
    }
}
